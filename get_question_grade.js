...
function get_exam_answer() {
	document.getElementById('back_button').addEventListener('click', () => {window.location = "view_exam_questions.html?title=" + encodeURIComponent(urlParams.get('exam_title')) + "&eid=" + encodeURIComponent(urlParams.get('eid'));});
...
function list_exam_submission(eq_json) {
	...
	document.getElementById('grade').textContent = eq_json.question.points_earned;
	document.getElementById('total_pts').textContent = eq_json.question.points_total;
